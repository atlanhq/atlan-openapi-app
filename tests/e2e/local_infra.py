...
class LocalProcessManager:
    ...
    def start_app(self, app: LocalAppConfig, *, handler: bool = True) -> None:
        ...
        worker_cmd = [
            "dapr", "run",
            "--app-id", f"local-{app.task_queue}",
            "--app-port", str(actual_worker_port),
            "--dapr-http-port", str(dapr_http_port),
            "--dapr-grpc-port", str(dapr_grpc_port),
            "--resources-path", self._dapr_components_path,
            "--log-level", "warn",
            "--placement-host-address", "",
            "--scheduler-host-address", "",
            "--",
            sys.executable, "-m", "app_framework.main",
            "--mode", "worker",
            "--app", app.module,
            "--task-queue", app.task_queue,
            "--health-port", str(actual_worker_port),
        ]
        procs.append(
            subprocess.Popen(
                worker_cmd,
        ...
        if has_handler:
            ...
            handler_cmd = [
                "dapr", "run",
                "--app-id", f"local-handler-{app.task_queue}",
                "--app-port", str(actual_handler_port),
                "--dapr-http-port", str(handler_dapr_http),
                "--dapr-grpc-port", str(handler_dapr_grpc),
                "--resources-path", self._dapr_components_path,
                "--log-level", "warn",
                "--placement-host-address", "",
                "--scheduler-host-address", "",
                ...
                "--app", app.module,
                "--task-queue", app.task_queue,
                "--handler-port", str(actual_handler_port),
            ]
            procs.append(
                subprocess.Popen(
                    handler_cmd,
